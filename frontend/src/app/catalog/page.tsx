...
const getCategoryGradient = (categoryName?: string) => {
  ...
  if (cat.includes('computer')) return 'from-indigo-900/90 via-slate-900 to-indigo-950 border-indigo-500/30';
  if (cat.includes('electrical')) return 'from-cyan-900/90 via-slate-900 to-blue-950 border-cyan-500/30';
  if (cat.includes('mechanical')) return 'from-amber-900/90 via-slate-900 to-orange-950 border-amber-500/30';
  if (cat.includes('math')) return 'from-emerald-900/90 via-slate-900 to-teal-950 border-emerald-500/30';
  if (cat.includes('physics')) return 'from-purple-900/90 via-slate-900 to-fuchsia-950 border-purple-500/30';
  if (cat.includes('business')) return 'from-blue-900/90 via-slate-900 to-sky-950 border-blue-500/30';
  if (cat.includes('econ')) return 'from-yellow-900/90 via-slate-900 to-amber-950 border-amber-500/30';
  if (cat.includes('med')) return 'from-teal-900/90 via-slate-900 to-emerald-950 border-teal-500/30';
  if (cat.includes('civil')) return 'from-stone-800 via-slate-900 to-stone-950 border-stone-500/30';
  if (cat.includes('hum') || cat.includes('lit')) return 'from-rose-900/90 via-slate-900 to-rose-950 border-rose-500/30';
  if (cat.includes('epic') || cat.includes('myth')) return 'from-amber-800/90 via-red-950 to-amber-950 border-amber-500/30';
  if (cat.includes('fiction')) return 'from-violet-900/90 via-slate-900 to-purple-950 border-violet-500/30';
  if (cat.includes('law')) return 'from-slate-900 via-amber-950/60 to-slate-950 border-amber-500/30';
  if (cat.includes('robot')) return 'from-cyan-900/90 via-indigo-950 to-slate-950 border-cyan-500/30';
  if (cat.includes('history')) return 'from-stone-900 via-amber-950 to-stone-950 border-amber-500/30';
  if (cat.includes('env')) return 'from-emerald-950 via-teal-900/80 to-slate-950 border-emerald-500/30';
  if (cat.includes('psych')) return 'from-fuchsia-950 via-purple-900/80 to-slate-950 border-fuchsia-500/30';
  if (cat.includes('space') || cat.includes('astro')) return 'from-sky-950 via-blue-900/80 to-slate-950 border-sky-500/30';
  return 'from-indigo-900/90 via-slate-900 to-purple-950 border-indigo-500/30';
};
        ...
        {isLoadingBooks ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {filteredBooks.map((book) => (
              <div
                key={book.id}
                className="border rounded-2xl overflow-hidden shadow-lg transition-all duration-300 hover:-translate-y-1.5 flex flex-col justify-between group"
                style={{ backgroundColor: 'var(--card-bg)', borderColor: 'var(--card-border)' }}
              >
                <div>
                  {/* Custom Themed Book Cover Banner (No External Images) */}
                  <div className={`h-60 w-full relative bg-gradient-to-br ${getCategoryGradient(book.categoryName)} p-5 flex flex-col justify-between overflow-hidden group/cover border-b border-white/10`}>
                    
                    {/* Decorative Background Watermarks */}
                    <BookOpen className="w-36 h-36 absolute -right-6 -bottom-6 text-white/5 font-black pointer-events-none transform -rotate-12 group-hover/cover:scale-110 transition-transform duration-500" />
                    <Sparkles className="w-16 h-16 absolute -left-4 -top-4 text-white/5 pointer-events-none" />

                    {/* Top Badges: Stock Status & Language Pill */}
                    <div className="flex items-center justify-between z-10">
                      ...
                      {book.language && (
                        <span className="px-2 py-0.5 rounded-md text-[10px] font-extrabold bg-white/10 text-white border border-white/20 backdrop-blur-sm uppercase tracking-wider">
                          {book.language}
                        </span>
                      )}
                    </div>

                    {/* Centerpiece Book Title & Author Typography */}
                    <div className="z-10 my-auto pr-2 space-y-1.5">
                      <span className="text-[10px] font-bold uppercase tracking-wider text-amber-300/90 block">
                        {book.categoryName || 'Academic Reference'}
                      </span>
                      <h2 className="text-base font-black text-white line-clamp-3 leading-snug tracking-tight drop-shadow-md group-hover/cover:text-amber-200 transition-colors">
                        {book.title}
                      </h2>
                      <p className="text-xs font-medium text-slate-200 line-clamp-1 drop-shadow-sm flex items-center gap-1 pt-0.5">
                        <span>by</span>
                    ...
                    <div className="z-10 flex items-center justify-between text-[10px] text-slate-300/80 font-mono border-t border-white/10 pt-2">
                      <span>ISBN: {book.isbn}</span>
                      <span>{book.publisher ? book.publisher.substring(0, 16) : ''}</span>
                    </div>

                    {/* Interactive Quick Action Hover Overlay */}
                    <div className="absolute inset-0 bg-slate-950/90 backdrop-blur-md opacity-0 group-hover/cover:opacity-100 transition-all duration-300 flex flex-col items-center justify-center gap-2.5 p-4 z-20">
                      <button
                        onClick={() => setSelectedBook(book)}
                        className="w-full py-2.5 rounded-xl text-xs font-extrabold bg-indigo-600 text-white shadow-lg shadow-indigo-600/40 hover:bg-indigo-500 transition flex items-center justify-center gap-1.5"
                      >
                      ...
                      {book.availableCopies === 0 ? (
                        <button
                          ...
                          className="w-full py-2.5 rounded-xl text-xs font-extrabold bg-amber-500 text-slate-950 shadow-lg shadow-amber-500/30 hover:bg-amber-400 transition flex items-center justify-center gap-1.5"
                        >
                        ...
                        <button
                          ...
                          className="w-full py-2.5 rounded-xl text-xs font-extrabold bg-emerald-600 text-white shadow-lg shadow-emerald-600/30 hover:bg-emerald-500 transition flex items-center justify-center gap-1.5"
                        >
                  ...
                  <div className="p-4 space-y-1.5">
                    <div className="flex items-center justify-between text-[11px]" style={{ color: 'var(--text-muted)' }}>
                      <span className="font-semibold text-indigo-400">{book.publisher || 'Published Edition'}</span>
                      <span className="font-mono text-[10px]">{book.publicationYear || ''}</span>
                    </div>

                    <p className="text-xs line-clamp-2" style={{ color: 'var(--text-muted)' }}>
                      {book.description || 'Verified academic literature reference in library collection.'}
                    </p>
                  </div>
                </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/75 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
          <div className="border rounded-2xl max-w-2xl w-full p-6 shadow-2xl space-y-6 my-8 relative" style={{ backgroundColor: 'var(--card-bg)', borderColor: 'var(--card-border)' }}>
            ...
            <div className="flex items-start space-x-4 border-b pb-4" style={{ borderColor: 'var(--card-border)' }}>
              <div className={`w-20 h-28 rounded-xl shadow-md shrink-0 bg-gradient-to-br ${getCategoryGradient(selectedBook.categoryName)} p-3 flex flex-col justify-between relative overflow-hidden border border-white/20`}>
                <BookOpen className="w-12 h-12 absolute -right-2 -bottom-2 text-white/10" />
                <span className="text-[9px] font-extrabold text-amber-300 uppercase tracking-wider line-clamp-1 z-10">{selectedBook.categoryName}</span>
                <p className="text-[11px] font-black text-white line-clamp-2 leading-tight z-10">{selectedBook.title}</p>
              </div>
